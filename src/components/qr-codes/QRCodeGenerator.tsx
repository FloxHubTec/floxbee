...
export function WhatsAppPhoneInput({ value, onChange }: any) {
    ...
    <div className="relative flex gap-2">
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 border rounded-md px-3 h-10 bg-background min-w-[120px]"
        >
        ...
        {open && (
          <div className="absolute z-50 mt-1 w-64 max-h-64 overflow-auto border rounded-md bg-background shadow">
            {getCountries().map((c) => {
              const FlagItem = flags[c];
      ...
      <input
        ...
        className="flex-1 rounded-md border px-3 h-10 text-sm"
      />
    </div>
  );
}
...
const QRCodeGenerator: React.FC<QRCodeGeneratorProps> = ({
  ...
  const [selectedLpSlug, setSelectedLpSlug] = useState(
    initialDados.landing_page_slug || "",
  );
  ...
  const generateContent = () => {
    ...
    switch (tipo) {
      ...
      case "whatsapp":{
        ...
        const finalPhone = `55${clean}`;
        content = generateQRContent.whatsapp(finalPhone, whatsappMessage);
